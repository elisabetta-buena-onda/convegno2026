...
export default function AdminPrenotazioni() {
  ...
  const startNew = () => {
    setEditingBooking({
      ...
      email: '',
      telefono: '',
      tipo_prenotazione: 'pernottamento_3_giorni',
                ...
                {loading ? (
                  <tr><td colSpan={7} className="p-12 text-center text-slate-500"><span className="material-symbols-outlined animate-spin mr-2 align-middle">sync</span>Caricamento...</td></tr>
                   ...
                   <tr key={b.id} className="hover:bg-slate-50/50 transition-colors group">
                     ...
                     <td className="px-4 py-4 align-top">
                       <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold inline-block border ${b.tipo_prenotazione.startsWith('pernottamento') ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                         {b.tipo_prenotazione.toUpperCase().replace('_', ' ')}
                       </span>
                       {b.struttura && (
                         <div className="mt-2 text-xs">
                           <p className="font-bold text-slate-700">{b.struttura}</p>
                           <p className="text-slate-500 capitalize">{b.alloggio}</p>
                         </div>
                       )}
                     </td>
                     <td className="px-4 py-4 align-top">
                     ...
                     <td className="px-4 py-4 align-top">
                       <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${
                         b.stato === 'CONFERMATA' ? 'bg-green-100 text-green-700' : 
              ...
              <form onSubmit={handleSave} className="p-6 space-y-6">
                 ...
                 <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                       <div className="space-y-1">
                        ...
                        <select className="w-full border border-slate-200 rounded-lg p-2 text-xs" value={editingBooking.tipo_prenotazione} onChange={e => setEditingBooking({...editingBooking, tipo_prenotazione: e.target.value})}>
                          <option value="pernottamento_3_giorni">Pernot. 3 Giorni</option>
                          <option value="pernottamento_2_giorni">Pernot. 2 Giorni</option>
                    ...
                    {editingBooking.struttura && (
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-400 uppercase">Sistemazione</label>
                        <select className="w-full border border-slate-200 rounded-lg p-2 text-xs" value={editingBooking.alloggio || ''} onChange={e => setEditingBooking({...editingBooking, alloggio: e.target.value})}>
                          <option value="">Seleziona...</option>
                          {accommodations.filter(a => a.structure.name === editingBooking.struttura).map(a => (
